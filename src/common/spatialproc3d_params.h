...
void set_projection_param(char * arg)
{
	...
	for (int i = 0; i < fields.size(); i++) {
		...
		if (selec.size() == 1) {
			...
			else if (stat_param.compare(PARAM_STATS_VOLUME_1) == 0) {	
...
/* Initialize default values in query structs (operator and temporary placeholders) */
	...
	int c;
	while ((c = getopt_long (argc, argv, "o:p:d:f:k:r:e:c:a:b:q:xs", long_options, &option_index)) != -1){
		switch (c)
		{
			case 0:
			...
			case 'b':
				...
				#endif
				// Must increase cardinality to 2
				stop.join_cardinality = 2;
				break;
